...
## 8.1 card组件
  在我们项目中，大量使用了卡片的展现形式，这个卡片有哪些功能点呢：
  1. 水平排列，垂直排列
  2. 显示内容标题，简要
就这么简单。
...
```
## 8.2 img-card组件
我们注意到除了首页的展示，其他主题日报也同样需要一个图片的首屏展示，展现的形式为图片+文字，文字浮于图片之上，此外还需要点击跳转对应的文章详情。我们同样也来开发一个img-card组件来满足这个需求。
`src/components/z-card/card.vue`
```html
<style lang="less">
  .img-card {
...
export default api;
```
在`build/webpack.base.conf.js`中添加添加别名，方便调用api：
```javascript
resolve: {
